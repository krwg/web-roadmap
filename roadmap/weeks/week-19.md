...
Ментальная модель Express — конвейер middleware. Каждый входящий запрос проходит по цепочке функций-станций (`logger` → `express.json()` → маршруты → обработка 404 → обработчик ошибок), и каждая станция может прочитать/изменить запрос, ответить сразу или передать управление дальше через `next()`. Порядок подключения middleware в коде буквально определяет порядок их выполнения — это не декларативная конфигурация, а императивная последовательность.
